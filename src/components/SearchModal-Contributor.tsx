  ...
  const SearchModal = ({ onClose, fieldType, onSelect, existingItems = [] }: SearchModalProps) => {
        ...
        try {
          ...
          switch (fieldType) {
            ...
            case "genres":
              ...
              break;
            case "students":
              endpoint = `${BASE_URL}/student/search`;
              break;
            default:
          ...
          const paginatedEndpoint = `${endpoint}?rpp=10&page=1`;
    ...
    const toggleItemSelection = (item: SearchItem) => {
      setSelectedItems((prevSelected) => {
        ...
        const isSelected = prevSelected.some((selectedItem) => 
          selectedItem.id === item.id
        );

        if (isSelected) {
          // Remove from selection by using filter to create new array that dont match
